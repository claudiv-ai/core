 ...
 */
export function projectFacets(dependency, sourceInterface) {
    let facets;
    if (dependency.facets && dependency.facets.length > 0) {
        // Filter to requested facets only
        facets = sourceInterface.facets.filter((f) => dependency.facets.includes(f.type));
    }
    else {
 ...
 */
export function resolveProjectedDependencies(component, currentScope, registry) {
 ...
 */
export function formatProjectedInterfaces(projections) {
    if (projections.length === 0)
        return '';
    const parts = ['## Dependency Interfaces'];
