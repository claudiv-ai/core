 ...
 */
export function resolveFQN(fqn, currentScope, registry) {
    ...
    if (registry.projects.has(firstSegment)) {
        ...
        return resolveAbsolute({ ...fqn, project: firstSegment, segments: fqn.segments.slice(1) }, registry);
    }
 ...
 */
export function buildFQN(elementName, ancestors, projectName) {
    const parts = [];
    if (projectName) {
 ...
 */
export function stringifyFQN(fqn) {
    const parts = [];
    if (fqn.project) {
 ...
 */
export function isAbsoluteFQN(fqn, registry) {
    return (fqn.project !== undefined ||
        registry.projects.has(fqn.segments[0]) ||
        fqn.segments[0] === registry.currentProject);
}
// ─── Internal Resolution ────────────────────────────────────────
function resolveAbsolute(fqn, registry) {
...
function resolveRelative(fqn, currentScope, registry) {
    ...
    throw new Error(`Cannot resolve FQN "${fqn.raw}" from scope [${currentScope.join(':')}]`);
}
function buildResolvedRef(fqn, component) {
    const ref = {
        fqn,
        file: component.file,
        component,
...
function resolveFragment(fqn, component) {
