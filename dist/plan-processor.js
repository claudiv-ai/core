 ...
 */
export function detectPlanDirectives($) {
    const directives = [];
 ...
 */
export function parsePlanQuestions($) {
    const questions = [];
 ...
 */
export function allQuestionsAnswered(questions) {
    return questions.every((q) => q.answer !== undefined && q.answer.trim() !== '');
}
/**
 * Generate plan:questions CDML block from question definitions.
 */
export function generatePlanQuestions(questions) {
    const lines = ['<plan:questions>'];
 ...
 */
export function questionsToFacts(questions, decisionSource) {
    return questions
        .filter((q) => q.answer)
        .map((q) => ({
 ...
 */
export function buildPlanPrompt(directive) {
    const parts = [];
...
// ─── Internal ───────────────────────────────────────────────────
function getExistingChildNames($, element) {
    const names = [];
...
function buildScopePath($, element) {
    const parts = [];
    let current = element;
    while (current && current.type === 'tag') {
        parts.unshift(current.name);
        const parentResult = $(current).parent();
...
function extractOptions($, element) {
    const options = [];
    $(element).children().each((_, child) => {
        if (child.type === 'tag' && child.name !== 'answer') {
            options.push($(child).text().trim());
        }
    });
    return options;
}
