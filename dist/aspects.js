 ...
 */
export function parseAspect(content, filePath) {
 ...
 */
export async function discoverAspects(searchDirs) {
    const aspectsByComponent = new Map();
    for (const dir of searchDirs) {
        if (!existsSync(dir))
            continue;
        const entries = await readdir(dir, { withFileTypes: true });
        for (const entry of entries) {
            if (!entry.isFile() || !entry.name.endsWith('.cdml'))
                continue;
 ...
 */
export function getAspectRelevantFacets(aspectType) {
    switch (aspectType) {
        case 'infra':
        case 'infrastructure':
 ...
 */
export function linkAspects(components, aspects) {
...
// ─── Internal ───────────────────────────────────────────────────
function extractAspectType(fileName) {
...
function inferAspectType(filePath) {
