...
function parseComponent($) {
    ...
    const fqn = fqnAttr ? parseFQN(fqnAttr) : {
        segments: [name],
        raw: name,
    };
    return {
 ...
 */
export function extractScopeConstraints($, element) {
    const constraints = [];
 ...
 */
export function buildPromptContext(component) {
    const parts = [];
    ...
    if (component.constraints) {
        ...
        if (component.constraints.os)
            parts.push(`  OS: ${component.constraints.os}`);
        if (component.constraints.arch)
            parts.push(`  Arch: ${component.constraints.arch}`);
        parts.push('');
    }
